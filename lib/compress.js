...
const compressUsingBestFormat = async ({ accepts, source, limits, width }) => {
...
const compressToSsim = async ({ source, format, limits, width }) => {
  ...
  detectedWidth = (await sourceImg.metadata()).width
  height = await sourceImg.metadata().height
  sourcePixels = [...await sourceImg.raw().toBuffer()]
