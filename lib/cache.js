...
const startJob = async options => {
  ...
  if (runningJobs.has(serializedOptions)) {
    return
  } else {
    runningJobs.add(serializedOptions)
  }
  ...
  runningJobs.delete(serializedOptions)
...
const retrieve = async (options, log = true) => {
  ...
  try {
    ...
    // NO AWAIT HERE. This is so we can do other stuff while compressing
    startJob({ source, format, width, quality })
