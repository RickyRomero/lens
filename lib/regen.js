...
const processEntry = async entry => {
  ...
  // The log is append-only and predates width validation on the server, so it
  // can contain entries recorded from junk query strings (?w=abc became NaN).
  // Those can never succeed; don't spend a Lambda invocation discovering that.
  if (!source || !Number.isFinite(Number(width)) || Number(width) < 1) {
    log('skipped  unusable log entry:', entry.join('||'))
    return 'skipped'
  }

  const { accepts, quality, density } = parseOptions(options)
  if (!accepts.length || !quality || !density) {
    log('skipped  unparseable options:', entry.join('||'))
    return 'skipped'
  }
...
const run = async () => {
  ...
  const counts = { compressed: 0, cached: 0, skipped: 0, refused: 0, failed: 0 }
  ...
  const worker = async () => {
    while (queue.length > 0 && !aborted) {
      ...
      try {
        ...
        // A 4xx is the compressor correctly declining work -- an image too big
        // for the current memory, a malformed entry. It answered in
        // milliseconds and will answer identically next time, so it is not the
        // systemic breakage the breaker exists for. It must not count: the log
        // is sorted, so one project's oversized variants sit consecutively and
        // would trip the breaker by themselves, stopping a run that is
        // otherwise succeeding.
        if (e.permanent) {
          counts.refused++
          logError(`refused  ${entry.join('||')} - ${e.message.split(' — ')[0]}`)
          continue
        }
  ...
  log(`finished: ${counts.compressed} compressed, ${counts.cached} already cached, ` +
    `${counts.skipped} skipped, ${counts.refused} refused, ${counts.failed} failed` +
    (aborted ? `, ${queue.length} not attempted` : ''))

  if (counts.refused > 0) {
    log(`  ${counts.refused} entries were declined as too large for the current ` +
      'MemorySize. Raise MemorySize and MAX_OUTPUT_MEGAPIXELS, then re-run to pick them up.')
  }

  // Refusals are a stable property of the current configuration, not a fault,
  // so they must not leave a cooldown marker blocking the next run.
