...
const run = async () => {
  ...
  for (const entry of entries) {
    const [source, format, width, quality, density] = entry
    ...
    try {
      console.log(source, format, width, quality, density)

      await fs.access(cacheTarget) // Check that the file doesn't exist first
    } catch (e) {
      await startJob({ source, format, width: Number(width), quality, density }, false)
    }
  }
